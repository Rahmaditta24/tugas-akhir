...
export default function Edit({ permasalahan, filters }) {
    const [values, setValues] = useState({
        ...
        provinsi: permasalahan?.provinsi || 'null',
        kabupaten_kota: permasalahan?.kabupaten_kota || 'null',
        jenis_permasalahan: permasalahan?.jenis_permasalahan || 'null',
        nilai: permasalahan?.nilai || 'null',
        satuan: permasalahan?.satuan || 'null',
        metrik: permasalahan?.metrik || 'null',
    ...
    const submit = (e) => {
        e.preventDefault();
        setProcessing(true);
        router.put(route('admin.permasalahan.update', { id: permasalahan.id, ...filters }), values, {
    ...
    return (
        <AdminLayout title="">
            <PageHeader
                ...
                actions={<Link href={route('admin.permasalahan.index', filters)} className="px-3 py-2 text-slate-700 bg-slate-100 rounded-md">Kembali</Link>}
            />
            ...
            <form onSubmit={submit} className="glass-card rounded-xl p-6 max-w-3xl">
                ...
                <div className="mt-6 flex gap-3">
                    ...
                    <Link href={route('admin.permasalahan.index', filters)} className="px-4 py-2 bg-slate-200 text-slate-700 rounded-md">Batal</Link>
                </div>
            </form>
        </AdminLayout>
    );
