            ...
            <div className="bg-white rounded-2xl w-full max-w-3xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden ring-1 ring-slate-200">
                ...
                <div className="flex-none px-4 sm:px-6 py-3 sm:py-4 border-b border-slate-100 flex justify-between items-center bg-white z-10">
                    <div>
                        ...
                        <p className="text-xs sm:text-sm text-slate-600 font-medium mt-0.5">Mengedit {items.length} data sekaligus</p>
                    </div>
                </div>
                        ...
                        <div key={item.id} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden relative transition-all hover:shadow-md hover:border-blue-200">
                            ...
                            <div className="bg-slate-50/80 px-3 sm:px-4 py-2 sm:py-3 border-b border-slate-100 flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h4 className="font-bold text-slate-800 text-xs sm:text-sm">{item.nama_pengusul || item.nama}</h4>
                                        <p className="text-[10px] sm:text-xs text-slate-500 font-medium">DATA ID: #{item.id}</p>
                                    </div>
                                </div>
                            </div>
                ...
                <div className="flex-none px-4 sm:px-6 py-3 sm:py-4 bg-slate-50 border-t border-slate-200 flex justify-between items-center">
                    ...
                    <div className="flex gap-2 sm:gap-3 w-full sm:w-auto">
                        <button
                            onClick={onClose}
                            className="flex-1 sm:flex-none px-4 sm:px-6 py-2 sm:py-2.5 bg-white text-slate-700 text-sm font-semibold rounded-lg hover:bg-slate-50 border border-slate-300 transition-colors shadow-sm"
                        >
                            Batal
                        </button>
                        <button
                            onClick={onSave}
                            disabled={isSaving}
                            className="flex-1 sm:flex-none px-4 sm:px-6 py-2 sm:py-2.5 bg-amber-500 text-white text-sm font-semibold rounded-lg hover:bg-amber-600 transition-all shadow-sm shadow-amber-200 active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                        >
                            {isSaving ? (
                                <><span className="w-3 h-3 sm:w-4 sm:h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></span> Menyimpan...</>
                            ) : (
